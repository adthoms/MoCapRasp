...
for ID, cameraPose in enumerate(cameras):
    ...
    sim.setObjectMatrix(
        cameraHandle, -1, cameraPose.tolist()
    )  # Put camera to its respective pose
    sim.setObjectOrientation(
        cameraHandle, list([0, 0, np.pi]), cameraHandle
    )  # Rotate camera object to match Coppelia's image display
    sim.setModelProperty(
        cameraHandle,
        sim.getModelProperty(cameraHandle) & ~sim.modelproperty_not_visible,
    )  # Make camera visible

# Show marker animation
for points in markers:
    # Insert points into point cloud
    totalPointCnt = sim.insertPointsIntoPointCloud(
        markersPointCloud, 0, points, markersColor
    )
...
sim.setModelProperty(
    quadcopterHandle,
    sim.getModelProperty(quadcopterHandle) & ~sim.modelproperty_not_visible,
)  # Make Quadcopter visible
...
sim.setModelProperty(
    quadcopterHandle,
    sim.getModelProperty(quadcopterHandle) | sim.modelproperty_not_visible,
)  # Make Quadcopter invisible again

# Disable camera visibility
for ID, cameraPose in enumerate(cameras):
    cameraHandle = sim.getObject(f"/Camera[{ID}]")
    sim.setModelProperty(
        cameraHandle, sim.getModelProperty(cameraHandle) | sim.modelproperty_not_visible
