...
def plotArena(title, allPoints3d, cameraData=None, groundData=None):
    ...
    ax.get_proj = lambda: np.dot(Axes3D.get_proj(ax), np.diag([1.0, 1.0, 0.5, 1.0]))
    colours = [
        ["fuchsia", "plum"],
        ["darkorange", "gold"],
    ...
    if cameraData is not None:
        for cam in range(len(projectionMatrices)):
            ...
            x, y, z = (
                np.matmul(projectionMatrices[cam], x),
                np.matmul(projectionMatrices[cam], y),
                np.matmul(projectionMatrices[cam], z),
            )
            # Adjust camera location
            o = np.matmul(projectionMatrices[cam], [[0.0], [0.0], [0.0], [1]]).ravel()
            ...
            if groundData is not None:
                ...
                x, y, z = (
                    np.matmul(planeRotation, x),
                    np.matmul(planeRotation, y),
                    np.matmul(planeRotation, z),
                )
            # Plot camera pose
            ax.quiver(
                o[0],
            ...
            ax.scatter(
                ...
                o[1],
                s=50,
                edgecolor=colours[cam][0],
                facecolor=colours[cam][1],
                linewidth=2,
                label="Camera " + str(cam),
            )
    ...
    if groundData is not None:
        if planeCoefficients is not None:
            ...
            X, Z = np.meshgrid(x, z)
            Y = (
                -planeCoefficients[0] * X - planeCoefficients[2] * Z
            ) / planeCoefficients[1]
            print("[RESULTS] Maximum Y:", max(np.array(Y).ravel()))
            print("[RESULTS] Minimum Y:", min(np.array(Y).ravel()))
            # Plot surface
            surf = ax.plot_surface(
                X, Z - cameraHeight, Y, color="b", alpha=0.15, label="Wand's plane"
            )
