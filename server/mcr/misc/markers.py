...
def activateNeedsOrder(nCameras, idx, needsOrder, relateLast2First=False):
    if not idx:
        ...
        if relateLast2First:
            ...
            needsOrder[str(nCameras - 1)] = np.unique(
                np.hstack((needsOrder[str(nCameras - 1)], [0]))
            )
        else:
            needsOrder[str(idx)] = np.array([idx + 1])

        # Add to the neighbor camera that idx needs ordering
        needsOrder[str(idx + 1)] = np.unique(
            np.hstack((needsOrder[str(idx + 1)], [idx]))
        )
    elif idx == (nCameras - 1):
        if relateLast2First:
...
def getTheClosest(coordNow, prev):
    ...
    centerCoord, prevCenterCoord = np.copy(coordNow).reshape(-1, 2), np.copy(
        prev
    ).reshape(-1, 2)
    newOrder, nMarkers = (
        np.ones(centerCoord.shape[0], dtype=np.int8) * -1,
        centerCoord.shape[0],
    )
    ...
    for i in range(nMarkers):
        ...
        if newOrder[i] == -1:
            ...
            if np.sum(retNow) == 1:
                ...
                allPermuationsOf4, allDists = (
                    np.array(list(permutations(list(range(0, 4))))),
                    [],
                )
...
def getOrderPerEpiline(coord1, coord2, nMarkers, F, verbose=0, retValue=0):
    # Get data
    pts1, pts2, orderSecondFrame = (
        np.copy(coord1).reshape(-1, 2),
        np.copy(coord2).reshape(-1, 2),
        np.ones(nMarkers, dtype=np.int8) * -1,
    )
    ...
    if len(choosenIdx) > 1:
        # Initiate variables
        allCombinationsOf2 = np.array(
            list(combinations(list(range(0, len(choosenIdx))), 2))
        )
        ...
        # Sort per X
        if np.all(np.diff(np.sort(collinearPts2[:, 0])) > 2) and np.all(
            np.diff(np.sort(collinearPts1[:, 0])) > 2
        ):
            order1, order2 = np.argsort(collinearPts1[:, 0]), np.argsort(
                collinearPts2[:, 0]
            )
            idx1 = np.hstack(
                (collinearPts1, idxAmbiguous1.reshape(idxAmbiguous1.shape[0], -1))
            )[order1, -1].T
            idx2 = np.hstack(
                (collinearPts2, idxAmbiguous2.reshape(idxAmbiguous2.shape[0], -1))
            )[order2, -1].T
        else:  # Sort per Y
            order1, order2 = np.argsort(collinearPts1[:, 1]), np.argsort(
                collinearPts2[:, 1]
            )
            idx1 = np.hstack(
                (collinearPts1, idxAmbiguous1.reshape(idxAmbiguous1.shape[0], -1))
...
def orderCenterCoord(centerCoord, prevCenterCoord, otherCamOrder=0):
    ...
    if len(prevCenterCoord) == 0:
        ...
        # Sort centers
        if np.linalg.norm(centerX[order[0]] - centerX[order[1]]) > np.linalg.norm(
            centerX[order[2]] - centerX[order[1]]
        ):
            sortedCenterCoord = np.array(
                (centerCoord[order[0]], centerCoord[order[1]], centerCoord[order[2]])
            )
        else:
            sortedCenterCoord = np.array(
                (centerCoord[order[2]], centerCoord[order[1]], centerCoord[order[0]])
            )
    else:
        ...
        distA = np.sqrt(
            np.power(np.subtract(prevCenterX[0], centerX), 2)
            + np.power(np.subtract(prevCenterY[0], centerY), 2)
        )
        distB = np.sqrt(
            np.power(np.subtract(prevCenterX[1], centerX), 2)
            + np.power(np.subtract(prevCenterY[1], centerY), 2)
        )
        ...
        # Sort centers
        sortedCenterCoord = [
            centerCoord[nearestA],
            centerCoord[nearestB[0]],
            centerCoord[nearestC],
        ]
        ...
        if (order[1] != 1) or (order[2] != prevOrder[2]):
            if prevOrder[0] == 2:
                order = swapElements(order, 0, 2)  # if is decreasing, swap
            sortedCenterCoord = np.array(
                (
                    sortedCenterCoord[order[0]],
                    sortedCenterCoord[order[1]],
