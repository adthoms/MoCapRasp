...
def undistortFisheye(points, intrisicsMatrix, distortionCoeffs):
    ...
    # Remove destortion
    undistortedPtsNorm = undistortPoints(
        pointsReshaped, intrisicsMatrix, distortionCoeffs
    )
...
def decomposeEssentialMat(E, K1, K2, pts1, pts2, cv2_compute=False, log=None):
    ...
    numPoints = pts1.shape[0]
    P1 = np.hstack((K1, [[0.0], [0.0], [0.0]]))

    for i in range(0, Ts.shape[0]):
        P2 = np.matmul(K2, np.hstack((Rs[i], Ts[i].T)))
        M1, M2 = P1[0:3, 0:3], P2[0:3, 0:3]
        c1, c2 = np.matmul(-np.linalg.inv(M1), P1[0:3, 3]), np.matmul(
            -np.linalg.inv(M2), P2[0:3, 3]
        )
        u1, u2 = np.vstack((pts1.T, np.ones((1, numPoints)))), np.vstack(
            (pts2.T, np.ones((1, numPoints)))
        )
...
def findRandT(ptA, ptB):
    ...
    centroidA, centroidB = (
        np.sum(ptA, axis=1).reshape(-1, 1) / nMarkers,
        np.sum(ptB, axis=1).reshape(-1, 1) / nMarkers,
    )
...
def getOtherValidIdx(line, nMarkers, idx, relateLast2First=0):
    ...
    nColumns = 2 * nMarkers
    valid = [
        np.any(line[i * nColumns : i * nColumns + nColumns])
        for i in [(idx - 1) % nCameras, (idx + 1) % nCameras]
    ]
