...
class CEC(CaptureProcess):
    ...
    def collect(self):
        ...
        counter, lastTime = np.zeros(self.cameras, dtype=np.uint16), np.zeros(
            self.cameras, dtype=np.uint32
        )
        missed, invalid = np.zeros(self.cameras, dtype=np.uint32), np.zeros(
            self.cameras, dtype=np.uint32
        )
        swap, certainty = np.zeros(self.cameras, dtype=np.uint16), np.zeros(
            self.cameras, dtype=np.bool8
        )
        ...
        try:
                ...
                if capture[idx]:  # Check if message is valid
                    if sizeMsg < 13:  # If less than 3 blobs, discard
                        ...
                        if sizeMsg > 13:
                            ...
                            coord = np.array(
                                [
                                    coord[orderAscDiameters[-1]],
                                    coord[orderAscDiameters[-2]],
                                    coord[orderAscDiameters[-3]],
                                ]
                            ).reshape(-1, 2)
                        ...
                        if self.save:
                            dfSave.append(
                                np.concatenate(
                                    (undCoord.reshape(6), [time, imgNumber, idx])
                                )
                            )
                        ...
                        if counter[idx]:
                            if abs(time - lastTime[idx]) > 1e9:
                                if self.verbose:
                                    print(
                                        "[WARNING-CAM+" + str(idx) + "] time missmatch"
                                    )
                                missed[idx] += 1
                                invalid[idx] += 1
                                continue
                        ...
                        if imgNumber > lastImgNumber[idx] + 1:
                            invalid[idx] = imgNumber - lastImgNumber[idx]

                        # Order markers per proximity and check collinearity
                        if (
                            isCollinear(*undCoord)
                            and not occlusion(undCoord, 5)
                            and not np.any(undCoord < 0)
                        ):
                            if invalid[idx] >= 10 or not counter[idx]:
                                if certainty[idx]:
                                    beg, end = intervals[idx][-1], counter[idx] - 1
                                    timeIntervals[idx].append(
                                        [dfOrig[idx][beg, 6], dfOrig[idx][end, 6]]
                                    )
                                    if self.verbose:
                                        print(
                                            "[WARNING-CAM+"
                                            + str(idx)
                                            + "] valid from "
                                            + str(round(dfOrig[idx][beg, 6] / 1e6, 2))
                                            + "s to "
                                            + str(round(dfOrig[idx][end][6] / 1e6, 2))
                                            + "s"
                                        )
                                ...
                                if not (counter[idx] - 1):
                                    prev = np.array(dfOrig[idx][0:6]).reshape(-1, 2)
                                else:
                                    prev = np.array(dfOrig[idx][-1, 0:6]).reshape(-1, 2)
                            ...
                            if self.verbose:
                                print(
                                    "[WARNING-CAM+"
                                    + str(idx)
                                    + "] not collinear or equal centroids"
                                )
                            missed[idx] += 1
                            invalid[idx] += 1
                            continue
                        ...
                        lastTime[idx], lastImgNumber[idx], invalid[idx] = (
                            time,
                            imgNumber,
                            0,
                        )
                        if not counter[idx]:
                            dfOrig[idx] = np.hstack((undCoord.reshape(6), time))
                        else:
                            dfOrig[idx] = np.vstack(
                                (dfOrig[idx], np.hstack((undCoord.reshape(6), time)))
                            )
                        counter[idx] += 1
                            ...
                            for [A, B, C] in undCoord.reshape([-1, 3, 2]):
                                if (
                                    ...
                                    if swap[idx] > 2:
                                        ...
                                        (
                                            dfOrig[idx][
                                                intervals[idx][-1] : counter[idx], 0:2
                                            ],
                                            dfOrig[idx][
                                                intervals[idx][-1] : counter[idx], 4:6
                                            ],
                                        ) = np.copy(
                                            dfOrig[idx][
                                                intervals[idx][-1] : counter[idx], 4:6
                                ...
                                if (
                                    np.linalg.norm(C - B) / np.linalg.norm(A - B)
                                    > (2 - tol)
                                    and np.linalg.norm(C - B) > 20
                                ):
                                    certainty[idx] = True
            ...
            if self.save:
                ...
                np.savetxt(
                    path + "CEC-" + HMS + ".csv", np.array(dfSave), delimiter=","
                )

            # Get last interval
            for idx in range(self.cameras):
                if not len(dfOrig[idx]):
                    continue
                if certainty[idx]:
                    beg, end = intervals[idx][-1], counter[idx] - 1
                    timeIntervals[idx].append(
            ...
            for cam in range(self.cameras - 1):
                ...
                intersections = [
                    ...
                    for first in timeIntervals[cam]
                    for second in timeIntervals[cam + 1]
                ...
                for [beg, end] in intersections:
                    for idx in range(cam, cam + 2):
                        validIdx = [
                            i
                            for i in range(0, len(dfOrig[idx]))
                            if beg <= dfOrig[idx][i, -1] <= end
                        ]
                        coord, time = (
                            dfOrig[idx][validIdx, 0:6],
                            dfOrig[idx][validIdx, 6] / 1e6,
                        )
                        if time.shape[0] <= 2:
                            continue
                        lowBound, highBound = math.ceil(
                            time[0] / self.step
                        ), math.floor(time[-1] / self.step)
                        if self.verbose:
                            print(
                                "[INFO] interpolated #"
                                + str(idx)
                                + " from "
                                + str(round(lowBound * self.step, 2))
                                + "s to "
                                + str(round(highBound * self.step, 2))
                                + "s"
                            )
                        tNew = np.linspace(
                            lowBound,
                        ...
                        dfInterp[
                            tNew, int((idx - cam) * 6) : int((idx - cam) * 6 + 6)
                        ] = ff(tNew * self.step)
                ...
                dfInterp = np.delete(
                    ...
                    np.unique(
                        [
                            ...
                            for idx in range(2)
                            if not np.any(dfInterp[i][idx * 6 : idx * 6 + 6])
                        ]
                    ),
                    axis=0,
                )
                if dfInterp.shape[0] < 10:
                    print(
                        "[ERROR] no valid image intersection for cameras "
                        + str(cam)
                        + " and "
                        + str(cam + 1)
                    )
                    return
                centroids1, centroids2 = dfInterp[:, 0:6].reshape(-1, 2), dfInterp[
                    :, 6:12
                ].reshape(-1, 2)
                print(
                    "[WARNING] interpolated "
                    + str(dfInterp.shape[0])
                ...
                print(
                    "[INFO] Computing fundamental and essential matrix between cameras "
                    + str(cam)
                    + "-"
                    + str(cam + 1)
                )
                try:
                    F, _ = estimateFundMatrix_8norm(
                        np.array(centroids1), np.array(centroids2), verbose=False
                    )
                    E = np.matmul(
                        self.cameraMat[cam + 1].T, np.matmul(F, self.cameraMat[cam])
                    )
                ...
                P1, P2 = np.hstack(
                    (self.cameraMat[cam], [[0.0], [0.0], [0.0]])
                ), np.matmul(self.cameraMat[cam + 1], np.hstack((R, t.T)))
                projPt1, projPt2 = projectionPoints(
                    np.array(centroids1)
                ), projectionPoints(np.array(centroids2))
                ...
                if points3d[0, 2] < 0:
                    points3d = -points3d
                tot, L_real_AC, L_real_AB, L_real_BC, k, false_idx = (
                    0,
                    15.7,
                    5.5,
                    10.2,
                ...
                print(
                    "\tImages distant more than 1% from the real value = "
                    + str(i)
                    + "/"
                    + str(int(points3d.shape[0] / 3))
                )
                ...
                centroids1, centroids2 = np.delete(
                    np.copy(centroids1), false_idx, axis=0
                ), np.delete(np.copy(centroids2), false_idx, axis=0)

                # Get fundamental and essential matrices
                F, _ = estimateFundMatrix_8norm(
                    np.copy(centroids1), np.copy(centroids2)
                )
                E = np.matmul(
                    self.cameraMat[cam + 1].T, np.matmul(F, self.cameraMat[cam])
                )
                ...
                projPt1, projPt2 = projectionPoints(
                    np.copy(centroids1)
                ), projectionPoints(np.copy(centroids2))
                points4d = triangulatePoints(
                    P1.astype(float),
                    P2.astype(float),
                    projPt1.astype(float),
                ...
                print(
                    "\tScale between real world and triang. point cloud is: ",
                    lamb.round(2),
                )
                print(
                    "\tL_AC >> mean = "
                    + str((np.mean(L_AC_vec) * lamb).round(4))
                    + "cm, std. dev = "
                    + str((np.std(L_AC_vec) * lamb).round(4))
                    + "cm, rms = "
                    + str(
                        (
                            np.sqrt(
                                np.mean(
                                    np.square(np.array(L_AC_vec) * lamb - L_real_AC)
                                )
                            )
                        ).round(4)
                    )
                ...
                print(
                    "\tL_AB >> mean = "
                    + str((np.mean(L_AB_vec) * lamb).round(4))
                    + "cm, std. dev = "
                    + str((np.std(L_AB_vec) * lamb).round(4))
                    + "cm, rms = "
                    + str(
                        (
                            np.sqrt(
                                np.mean(
                                    np.square(np.array(L_AB_vec) * lamb - L_real_AB)
                                )
                            )
                        ).round(4)
                    )
                ...
                print(
                    "\tL_BC >> mean = "
                    + str((np.mean(L_BC_vec) * lamb).round(4))
                    + "cm, std. dev = "
                    + str((np.std(L_BC_vec) * lamb).round(4))
                    + "cm, rms = "
                    + str(
                        (
                            np.sqrt(
                                np.mean(
                                    np.square(np.array(L_BC_vec) * lamb - L_real_BC)
                                )
                            )
                        ).round(4)
                    )
            ...
            for cam in range(self.cameras):
                ...
                for i in np.flip(range(cam + 1)):
                    t, R, lamb = (
                        np.array(translation[i][0]).reshape(-1, 3),
                        np.array(rotation[i]),
                        scale[i],
                    )
                ...
                if cam < self.cameras - 1:
                    points3d = np.hstack(
                        (
                            points3D_perPair[cam] * scale[cam + 1][0] / 100,
                            np.ones((points3D_perPair[cam].shape[0], 1)),
                        )
                    ).T
            ...
            np.savetxt(
                "mcr/capture/data/projMat.csv", np.array(projMat).ravel(), delimiter=","
            )
            ...
            plotArena(
                title="3D Map of the Calibration Process",
