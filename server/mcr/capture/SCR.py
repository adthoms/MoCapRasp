...
class SCR(CaptureProcess):
    ...
    def collect(self):
        ...
        counter, lastTime, lastImgNumber = (
            np.zeros(self.cameras, dtype=np.int32),
            np.zeros(self.cameras, dtype=np.int32),
            np.zeros(self.cameras, dtype=np.int32),
        )
        ...
        dfInterp = np.zeros(
            (self.nImages, self.cameras * (2 * self.markers) + 2)
        )  # Last two columns are special

        dfInterp[:, -2] = np.arange(
            0, self.record, self.step
        )  # Penultimate column is filled with the time stamps
        dfInterp[:, -1] = np.zeros_like(
            dfInterp[:, -1], dtype=np.bool8
        )  # Last column is filled with bool zeros (false)

        # Each PTS will have...
        dfTriang = np.zeros(
            (int(self.record / self.step), (4 * self.markers) + 1)
        )  # 4D coordinates of each marker + PTS
        dfTriang[:, -1] = np.arange(
            0, self.record, self.step
        )  # Last column is filled with the PTSs
        ...
        P_plane = np.genfromtxt("mcr/capture/data/P_plane.csv", delimiter=",").reshape(
            -1, 4
        )
        [d, b, h] = np.genfromtxt(
            "mcr/capture/data/groundData.csv", delimiter=","
        ).reshape(3)
        ...
        try:
            while np.any(capture):
                ...
                if capture[idx]:  # Check if message is valid
                    if (
                        sizeMsg < (3 * self.markers) + 4
                    ):  # If less than nMarker blobs are found, discard
                        if self.verbose:
                            print(
                                "[ERROR] "
                                + str(int((sizeMsg - 4) / 3))
                        ...
                        msg = message[0 : sizeMsg - 4].reshape(
                            -1, 3
                        )  # Separating blob info
                        coord, blobArea = msg[:, 0:2], msg[:, 2].reshape(
                            -1
                        )  # Separating coordinates

                        # If more than nMarker blobs are found, get the nMarker biggest
                        if sizeMsg > (3 * self.markers) + 4:
                            orderAscDiameters = np.argsort(blobArea)[
                                ::-1
                            ]  # Sorted by biggest to lowest diameter
                            coord = np.array(
                                [
                                    coord[biggestBlobs]
                                    for biggestBlobs in orderAscDiameters[
                                        : self.markers
                                    ]
                                ]
                            ).reshape(-1, 2)
                        ...
                        if self.save:
                            dfSave.append(
                                np.concatenate(
                                    (
                                        undCoord.reshape(2 * self.markers),
                        ...
                        if counter[idx]:
                            if abs(timeNow - lastTime[idx]) > 1e9:
                                ...
                                invalid[
                                    idx
                                ] += 1  # One more missed invalid for the camera
                                continue
                        ...
                        if not occlusion(undCoord, 5) and not np.any(undCoord < 0):
                            if invalid[idx] >= 10 or not counter[idx]:
                                ...
                                prev = []
                                needsOrder = activateNeedsOrder(
                                    self.cameras, idx, needsOrder, relateLast2First=0
                                )
                                intervals[idx].append(counter[idx])
                            else:
                                if not (counter[idx] - 1):
                                    prev = np.array(
                                        dfOrig[idx][0 : (2 * self.markers)]
                                    ).reshape(-1, 2)
                                else:
                                    prev = np.array(
                                        dfOrig[idx][-1, 0 : (2 * self.markers)]
                                    ).reshape(-1, 2)
                                newOrder = getTheClosest(
                                    undCoord.reshape(-1, 2), prev.reshape(-1, 2)
                                )
                        ...
                        if not counter[idx]:
                            dfOrig[idx] = np.hstack(
                                (undCoord.reshape(2 * self.markers), timeNow)
                            )
                        else:
                        ...
                        if needsOrder[str(idx)].size:
                            ...
                            for otherIdx in needsOrder[str(idx)]:
                                if not len(intervals[otherIdx]):
                                    continue
                                myCounter, myIntervals = np.array(
                                    [counter[idx], counter[otherIdx]]
                                ), np.array(
                                    [intervals[idx][-1], intervals[otherIdx][-1]]
                                )
                                if np.all(myCounter - myIntervals >= nPrevious):
                                    # See if there are intersection between arrays
                                    ts1 = (
                                        dfOrig[idx][
                                            intervals[idx][-1] : counter[idx],
                                            (2 * self.markers),
                                        ]
                                        / 1e6
                                    )
                                    ts2 = (
                                        dfOrig[otherIdx][
                                            intervals[otherIdx][-1] : counter[otherIdx],
                                            (2 * self.markers),
                                        ]
                                        / 1e6
                                    )
                                    ...
                                    validIdx1 = [
                                        k
                                        for k in range(0, len(ts1))
                                        if maxFirstIntersection - 0.01
                                        <= ts1[k]
                                        <= minLastIntersection + 0.01
                                    ]
                                    validIdx2 = [
                                        k
                                        for k in range(0, len(ts2))
                                        if maxFirstIntersection - 0.01
                                        <= ts2[k]
                                        <= minLastIntersection + 0.01
                                    ]
                                    ...
                                    if len(validIdx1) and len(validIdx2):
                                        ...
                                        ts1, ts2 = np.copy(ts1[validIdx1]), np.copy(
                                            ts2[validIdx2]
                                        )

                                        if ts1.shape[0] < 2 or ts2.shape[0] < 2:
                                            continue
                                        ...
                                        coord1, coord2 = (
                                            ...
                                            dfOrig[otherIdx][
                                                intervals[otherIdx][-1] : counter[
                                                    otherIdx
                                                ],
                                                0 : (2 * self.markers),
                                            ],
                                        )
                                        coord1, coord2 = np.copy(
                                            coord1[validIdx1]
                                        ), np.copy(coord2[validIdx2])

                                        # Get interpolated data
                                        interp1, tNew1 = interpolate(
                                            coord1, ts1, self.step
                                        )
                                        interp2, tNew2 = interpolate(
                                            coord2, ts2, self.step
                                        )
                                        if not len(interp1) or not len(interp2):
                                            continue

                                        # Get min and max idx
                                        minIdx, maxIdx = min(idx, otherIdx), max(
                                            idx, otherIdx
                                        )
                                        ...
                                        if idx < otherIdx:
                                            orderSecondFrame, ret = getOrderPerEpiline(
                                                interp1[-1],
                                                interp2[-1],
                                                self.markers,
                                                F,
                                        ...
                                        # Flip blobs
                                        for k in range(
                                            intervals[maxIdx][-1], counter[maxIdx]
                                        ):
                                            dfOrig[maxIdx][
                                                k, 0 : (2 * self.markers)
                                            ] = np.copy(
                                                ...
                                                .reshape(-1, 2)[orderSecondFrame]
                                                .reshape(-(2 * self.markers))
                                            )

                                        # Change ordering boolean
                                        needsOrder = popNeedsOrder(
                                            idx, otherIdx, needsOrder
                                        )
                        ...
                        if (counter[idx] - intervals[idx][-1]) >= warmUp:
                            ...
                            coord, ts = (
                                dfOrig[idx][
                                    (counter[idx] - warmUp) : counter[idx],
                                    0 : (2 * self.markers),
                                ],
                                dfOrig[idx][
                                    (counter[idx] - warmUp) : counter[idx],
                            ...
                            lowBound, highBound = math.ceil(
                                ts[0] / self.step
                            ), math.floor(ts[-1] / self.step)

                            # Avoid repeating intepolation
                            if lastTnew[idx]:
                                if lowBound < lastTnew[idx]:
                                    lowBound = lastTnew[idx] + 1
                            ...
                            dfInterp[
                                tNew,
                                int(idx * (2 * self.markers)) : int(
                                    idx * (2 * self.markers) + (2 * self.markers)
                                ),
                            ] = ff(tNew * self.step)
                            lastTnew[idx] = tNew[-1]
                            ...
                            for k in tNew:
                                if dfInterp[k, -1]:
                                    continue
                                otherIdx = getOtherValidIdx(
                                    dfInterp[k, :], self.markers, idx
                                )
                                if (
                                    not len(otherIdx)
                                    or otherIdx in needsOrder[str(idx)]
                                ):
                                    continue
                                ...
                                pts1, pts2 = dfInterp[
                                    k,
                                    minIdx
                                    * (2 * self.markers) : (minIdx + 1)
                                    * (2 * self.markers),
                                ].reshape(-1, 2), dfInterp[
                                    k,
                                    maxIdx
                                    * (2 * self.markers) : (maxIdx + 1)
                                    * (2 * self.markers),
                                ].reshape(
                                    -1, 2
                                )
                                R, t, lamb = (
                                    rotation[maxIdx],
                                    translation[maxIdx].reshape(-1, 3),
                                    scale[maxIdx],
                                )
                                P1, P2 = np.hstack(
                                ...
                                projPt1, projPt2 = projectionPoints(
                                    np.array(pts1)
                                ), projectionPoints(np.array(pts2))
                                ...
                                # Save to array
                                dfTriang[k, 0 : (4 * self.markers)] = np.copy(
                                    points3d.ravel()
                                )
            ...
            if self.save:
                ...
                np.savetxt(
                    path + "SCR-" + HMS + ".csv", np.array(dfSave), delimiter=","
                )

            # Just comprising dataset if wanted to plot
            emptyLines = np.unique(
                [i for i in range(0, dfInterp.shape[0]) if not dfInterp[i][-1]]
            )
            ...
            groundData = {"planeDisplacement": d / b, "planeRotation": P_plane}
